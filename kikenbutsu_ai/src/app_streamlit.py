...
with tabs[0]:
    ...
    if eq:
        ...
        for title, era, text, conf, review in rows:
            st.markdown(f"**通知名**: {title} / **年代**: {era} / **信頼度**: {conf:.2f}")
            st.markdown(f"**原本確認要否**: {'原本確認推奨' if review else '通常確認'}")
            st.code(text[:600])

with tabs[1]:
    era = st.selectbox("年代", ["昭和", "平成", "令和", "不明"])
    q = st.text_input("年代内キーワード")
    if st.button("年代検索"):
        rows = [r for r in engine.search_fts(q or era, 80) if r[2] == era or era == "不明"]
        for r in rows:
            st.write(f"通知名: {r[1]} / 年代: {r[2]} / 設備: {r[3]} / 信頼度: {r[5]:.2f}")
            st.code(r[4][:500])

with tabs[2]:
    q = st.text_input("条文検索（例: 第10条）")
    if q:
        rows = engine.search_fts(q, 40)
        for r in rows:
            st.write(f"通知名: {r[1]} / 年代: {r[2]} / 設備: {r[3]}")
            st.code(r[4][:500])

with tabs[3]:
    eq = st.text_input("差分検索設備（空欄なら全設備）")
    for name, topic, diff, old_t, new_t in engine.search_revisions(eq or None):
        st.markdown(f"### {name} / {topic}")
        st.write(f"差分要約: {diff}")
        with st.expander("旧基準"):
            st.code(old_t[:1200])
        with st.expander("新基準"):
            st.code(new_t[:1200])
...
with tabs[4]:
    ...
    if eq:
        ...
        for label, text, conf, review in reqs:
            ...
            st.code(text[:300])
        st.markdown("#### 関係条文一覧")
        for law, art, para, item, text in laws:
            st.write(f"- {law} {art} {'第'+para+'項' if para else ''} {'第'+item+'号' if item else ''}")
            st.caption(text[:220])
