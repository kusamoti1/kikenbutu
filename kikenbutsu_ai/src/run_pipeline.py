...
try:
    ...
    from .contextual_chunk_builder import build_contextual_chunks
    ...
    from .paragraph_splitter import split_paragraphs, split_paragraphs_with_confidence
    from .revision_diff_engine import summarize_diff, match_paragraphs
    ...
    from src.paragraph_splitter import split_paragraphs, split_paragraphs_with_confidence
    from src.revision_diff_engine import summarize_diff, match_paragraphs
...
def _insert_document_paragraphs(conn, cfg, pdf_path: Path, dictionary: dict[str, str]) -> DocData | None:
    ...
    line_rows: list[tuple[str, float]] = _run_internal_ocr(cfg, pdf_path)
    ...
    if line_rows:
        ...
        if not imported:
            logging.warning("OCR結果・キャッシュともに得られずスキップ: %s", pdf_path.name)
            return None
    ...
    if confidence_known:
        line_dicts = [{"text_original": t, "confidence": c} for t, c in line_rows]
        para_objs = split_paragraphs_with_confidence(
            line_dicts,
            threshold=cfg.ocr_confidence_threshold,
            min_len=cfg.min_paragraph_length,
        )
        raw_paras = [p["text_original"] for p in para_objs]
        conf_pairs = [(p["confidence_avg"], p["confidence_min"]) for p in para_objs]
    else:
        raw_paras = split_paragraphs(text_original)
        conf_pairs = [(None, None)] * len(raw_paras)

    norm_paras = [convert_old_kanji(apply_dictionary(p, dictionary)) for p in raw_paras]
    count = min(len(raw_paras), len(norm_paras))
    raw_paras = raw_paras[:count]
    norm_paras = norm_paras[:count]
    conf_pairs = conf_pairs[:count]
...
def _create_revisions(conn, docs_info: list[DocData]) -> int:
    ...
    for old_doc, new_doc, score in pairs:
        ...
        para_pairs = match_paragraphs(old_doc.paragraphs, new_doc.paragraphs, min_score=0.40)[:2]
...
def main() -> None:
    ...
    try:
        ...
        for pdf in pdf_files:
            try:
                ...
                if doc:
                    ...
                    # 重複またはOCR失敗をログ文面から判定
                    # （最小変更のため戻り値Noneを2種類で扱う）
                    txt_cache = _load_imported_text(cfg, pdf)
                    if txt_cache:
                        skipped_duplicate += 1
                    else:
                        ocr_fail += 1
        ...
        review_needed = conn.execute("SELECT COUNT(*) FROM paragraphs WHERE needs_review = 1").fetchone()[0]
        print(f"処理完了: 対象PDF {total_pdf}件 / 正常処理 {processed}件 / 重複等スキップ {skipped_duplicate}件 / OCR失敗 {ocr_fail}件 / 要確認段落 {review_needed}件 / 差分 {revision_count}件 / 出力 {len(exports)}件")
    finally:
        conn.close()
